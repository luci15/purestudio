...
-- 4. Cash Dealings Table (kept separate from GST invoices, no tax fields)
...
-- 6. Create RLS Policies to allow anonymous public read/write access
...
create policy "Allow all operations for anon on cash_entries" on public.cash_entries for all using (true) with check (true);

-- 7. Enable Realtime Publications for live multi-device syncing
